...
/// Set true when we are watering tiles
bool mFireActive = false;
/**
 * Constructor
...
{
    auto mGrabbedItem = mCity.HitTest(event.GetX(), event.GetY());
    if (mGrabbedItem != nullptr)
    {
        IsBuildingTileVisitor visitor;
        mGrabbedItem->Accept(&visitor);
        DoubleClickFireStation visitor1;
        mGrabbedItem->Accept(&visitor1);
        //if(visitor.IsBuilding()) {
            // The tile is a Landscape tile
...
//                Refresh();
//
//                SetFireVisitor visitor1;
//                mCity.Accept(&visitor1);
//
//                TileBuilding::SetOnFire(mGrabbedItem);
//
//            } else {
...
/**
 * Handle the Landscape>Watering menu option
 * @param event
 */
void CityView::OnBuildingFire(wxCommandEvent& event)
{
    mFireActive = !mFireActive;
}

/**
 * Update the checkmark for the Landscape>Watering menu option
 * @param event
 */
void CityView::OnUpdateBuildingFire(wxUpdateUIEvent& event)
{
    event.Check(mFireActive);
}
